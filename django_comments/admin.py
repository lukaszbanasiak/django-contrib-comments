...
from django_comments import get_admin
...
class BaseCommentAdmin(admin.ModelAdmin):
    ...
    def get_actions(self, request):
        actions = super(BaseCommentAdmin, self).get_actions(request)
...
class LegacyCommentAdmin(BaseCommentAdmin):
    fieldsets = (
        (None,
           {'fields': ('content_type', 'object_pk', 'site')}
        ),
...
class NonAuthCommentAdmin(BaseCommentAdmin):
    fieldsets = (
        (None,
           {'fields': ('content_type', 'object_pk', 'site')}
        ),
...
class AuthCommentAdmin(BaseCommentAdmin):
    fieldsets = (
        (None,
           {'fields': ('content_type', 'object_pk', 'site')}
        ),
        (_('Content'),
           {'fields': ('user', 'name', 'email', 'comment')}
        ),
        (_('Metadata'),
           {'fields': ('submit_date', 'ip_address', 'is_public', 'is_removed')}
        ),
    ...
    raw_id_fields = ('user',)
    search_fields = ('comment', UsernameSearch(), 'name', 'email', 'ip_address')
    readonly_fields = ('name', 'email')

Comment = get_model()
CommentAdmin = get_admin()
